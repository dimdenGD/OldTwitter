...
let userDataFunction = async user => {
    ...
    userAvatar.src = `${(user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): user.profile_image_url_https}`.replace('_normal.', '_bigger.');
                ...
                userElement.innerHTML = /*html*/`
                    <div>
                        <a href="https://twitter.com/${u.screen_name}" class="following-item-link">
                            <img src="${`${(u.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): u.profile_image_url_https}`}" alt="${u.screen_name}" class="following-item-avatar tweet-avatar" width="48" height="48">
        ...
        for(let i in lastConvo.entries) {
            ...
            messageElement.innerHTML = `
                ${sender.id_str !== user.id_str ? `
                    <a href="https://twitter.com/${sender.screen_name}"><img src="${`${(sender.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): sender.profile_image_url_https}`.replace("_normal", "_bigger")}" width="26" height="26"></a>
    ...
    function renderInboxMessages(inbox, inboxList) {
        ...
        for(let i in inbox.conversations) {
            ...
            messageElement.innerHTML = /*html*/`
                <img src="${messageUsers.length === 1 ? `${(messageUsers[0].default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): messageUsers[0].profile_image_url_https}` : (c.avatar_image_https || chrome.runtime.getURL(`/images/group.jpg`))}" width="48" height="48" class="inbox-message-avatar">
            ...
            messageElement.addEventListener('click', async () => {
                ...
                messageHeaderAvatar.src = messageUsers.length === 1 ? `${(messageUsers[0].default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): messageUsers[0].profile_image_url_https}` : (c.avatar_image_https || chrome.runtime.getURL(`/images/group.jpg`));
        ...
        userSearch.addEventListener('keyup', async () => {
            ...
            res.users.slice(0, 5).forEach(u => {
                ...
                userElement.innerHTML = `
                    <img class="new-message-user-avatar" src="${`${(u.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): u.profile_image_url_https}`.replace("_normal", "_bigger")}" width="48" height="48">
                ...
                userElement.addEventListener('click', async () => {
                    ...
                    messageHeaderAvatar.src = `${(u.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): u.profile_image_url_https}`;
    ...
    document.getElementById('navbar-tweet-button').addEventListener('click', () => {
        ...
        modal.getElementsByClassName('navbar-new-tweet-avatar')[0].src = `${(user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): user.profile_image_url_https}`.replace("_normal", "_bigger");
        ...
        newTweetText.addEventListener('keydown', async e => {
            ...
            if(/(?<!\w)@([\w+]{1,15}\b)$/.test(e.target.value)) {
                ...
                users.forEach((user, index) => {
                    ...
                    userElement.innerHTML = `
                        <img width="16" height="16" class="search-result-item-avatar" src="${`${(user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): user.profile_image_url_https}`}">
            ...
            div.innerHTML = /*html*/`
                ...
                <div class="preview-user-data">
                    <a class="preview-user-avatar-link" href="https://twitter.com/${user.screen_name}">
                        <img class="preview-user-avatar" width="50" height="50" src="${`${(user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): user.profile_image_url_https}`.replace('_normal.', '_400x400.')}">
                    </a>
                    <br>
