...
function generateCard(tweet, tweetElement, user) {
    ...
    if(tweet.card.name === 'promo_image_convo' || tweet.card.name === 'promo_video_convo') {
        ...
        iframe.src = tweet.card.binding_values.player_url.string_value.replace("youtube", "youtube-nocookie").replace("autoplay=true", "autoplay=false").replace("autoplay=1", "autoplay=0");
