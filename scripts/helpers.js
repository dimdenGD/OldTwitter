      ...
      if (base64Data.length === media.length) {
        ...
        base64Data.forEach((data) => {
          ...
          cw.addEventListener("click", () => {
            ...
            [graphic_violence, adult_content, sensitive_content].forEach(
              (checkbox) => {
    ...
    if (t.entities.hashtags) {
      t.entities.hashtags.forEach((hashtag) => {
        let hashflag = hashflags.find(
          (h) => h.hashtag.toLowerCase() === hashtag.text.toLowerCase()
        );
    ...
    if (t.entities.urls) {
      t.entities.urls.forEach((url) => {
        index_map[url.indices[0]] = [
          url.indices[1],
          (text) =>
            `<a href="${escapeHTML(url.expanded_url)}" title="${escapeHTML(
              url.expanded_url
    ...
    if (t.entities.user_mentions) {
      t.entities.user_mentions.forEach((user) => {
        index_map[user.indices[0]] = [
          user.indices[1],
          (text) =>
            `<a href="/${escapeHTML(user.screen_name)}">${escapeHTML(
              text
            )}</a>`,
        ];
      });
    ...
    new Promise(async (resolve, reject) => {
      ...
      chrome.storage.local.get(["legacyLinkColors"], async (data) => {
        ...
        let toFetch = ids.filter(
          (id) => !hasColourIds.includes(id) && !noColourIds.includes(id)
        );
        let users = [];
        if (toFetch.length > 0) {
          try {
...
function languageMatches(tweetLanguage) {
  ...
  if (
    ["qam", "qct", "qht", "qme", "qst", "zxx", "und"].includes(tweetLanguage)
  ) {
...
async function renderTrends(compact = false, cache = true) {
  ...
  let [trendsData, hashflags] = await Promise.allSettled([
    API.discover[vars.disablePersonalizedTrends ? "getTrends" : "getTrendsV2"](
      cache
    ),
  ...
  trends.slice(0, max).forEach(({ trend }) => {
    ...
    trendDiv.innerHTML = compact
      ? html`<a href="/search?q=${escapeHTML(trend.name)}" class="trend-name"
    ...
    usersSuggestions.forEach((userId) => {
      ...
      udiv.innerHTML = html`
        <a class="tweet-avatar-link" href="/${userData.screen_name}"
          ><img
            src="${`${
              userData.default_profile_image && vars.useOldDefaultProfileImage
...
function renderMedia(t) {
  ...
  if (cws.length > 0) {
    ...
    _html +=
      `<br>` + elNew("div", { className: "tweet-media-cws" }, [cws]).outerHTML;
  }
  return _html;
}
...
async function appendTweet(t, timelineContainer, options = {}) {
  ...
  try {
    ...
    if (tweetTranslate || tweetTranslateAfter) {
      (tweetTranslate ? tweetTranslate : tweetTranslateAfter).addEventListener(
        "click",
        async () => {
      ...
      if (
        ...
        if (!options || !options.top || !options.top.text)
          t.extended_entities.media.forEach((m) => {
            ...
            if (!vars.displaySensitiveContent && t.possibly_sensitive)
            ...
            if (typeof timeline !== "undefined") {
              let index = timeline.data.findIndex(
                (tweet) =>
                  ...
                  tweet.retweeted_status.id_str === t.id_str &&
      ...
      if (!options.mainTweet) {
        ...
        if (mainTweetLikers.find((liker) => liker.id_str === user.id_str)) {
          mainTweetLikers.splice(
            mainTweetLikers.findIndex((liker) => liker.id_str === user.id_str),
            1
          );
    ...
    t.renderFavoritesUp = () => {
      ...
      if (!options.mainTweet) {
        ...
        if (
          ...
          !mainTweetLikers.find((liker) => liker.id_str === user.id_str)
        ) {
          let a = document.createElement("a");
    ...
    if (t.user.id_str === user.id_str) {
      ...
      tweetInteractMoreMenuDelete.addEventListener("click", async () => {
        ...
        Array.from(
          document.querySelectorAll(`div.tweet[data-tweet-id="${t.id_str}"]`)
        ).forEach((tweet) => {
          tweet.remove();
        });
        if (options.mainTweet) {
          ...
          if (tweets.length === 0) {
            ...
            location.href =
              tweets[0].getElementsByClassName("tweet-time")[0].href;
          }
        }
        if (typeof timeline !== "undefined") {
        ...
        tweetInteractMoreMenuPin.addEventListener("click", async () => {
          if (pinnedTweet && pinnedTweet.id_str === t.id_str) {
            ...
            let beforeTweet = Array.from(
              ...
              let timestamp =
                +i.getElementsByClassName("tweet-time")[0].dataset.timestamp;
        ...
        t.extended_entities.media.forEach((item, index) => {
          ...
          _fetch(url)
            ...
            .then((blob) => {
              ...
              let _index =
                t.extended_entities.media.length > 1 ? "_" + (index + 1) : "";
        ...
        feedbackButton.addEventListener("click", () => {
          ...
          if (
            ...
            fetch(`/i/api/graphql/OiKldXdrDrSjh36WO9_3Xw/TopicNotInterested`, {
              ...
              body: JSON.stringify({
                variables: {
                  topicId: feedback.richBehavior.markNotInterestedTopic.topicId,
                  undo: false,
                },
    ...
    notification.addEventListener("click", (e) => {
      if (
        ...
        if (n.icon.id === "bell_icon") {
          ...
          if (
            n.entry.clientEventInfo.element === "user_liked_multiple_tweets"
          ) {
            let a = document.createElement("a");
            ...
            new TweetViewer(
              user,
              n.tweet.retweeted_status ? n.tweet.retweeted_status : n.tweet
            );
          }
        } else if (
          n.entry.clientEventInfo.element === "users_added_you_to_lists"
        ) {
          let a = document.createElement("a");
          ...
          n.entry.clientEventInfo.element === "follow_from_recommended_user"
        ) {
          let a = document.createElement("a");
          ...
          setTimeout(async () => {
            document.getElementsByClassName("navbar-new-tweet-text")[0].value =
              LOC.anniversary_tweet.message;
            ...
            handleFiles(
              [anniversaryPicFile],
              mediaToUpload,
              document.getElementsByClassName("navbar-new-tweet-media-c")[0]
            );
          }, 10);
          ...
          .map(
            (u) =>
              `<a class="notification-avatar" href="/${
                ...
                !u.profile_image_url_https && vars.useOldDefaultProfileImage
                  ? chrome.runtime.getURL(
                      `images/default_profile_images/default_profile_${
                        Number(u.id_str) % 7
                      }_normal.png`
                  ...
                  : u.profile_image_url_https
              }`.replace("_normal", "_bigger")}" alt="${escapeHTML(
                u.name
