...
async function renderDiscovery(cache = true) {
    ...
    try {
        ...
        usersSuggestions.forEach(userId => {
            ...
            udiv.innerHTML = `
                <a class="tweet-avatar-link" href="https://twitter.com/${userData.screen_name}"><img src="${`${(userData.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): userData.profile_image_url_https}`.replace("_normal", "_bigger")}" alt="${escapeHTML(userData.name)}" class="tweet-avatar" width="48" height="48"></a>
    ...
    userElement.innerHTML = `
        <div>
            <a href="https://twitter.com/${u.screen_name}" class="user-item-link">
                <img src="${(u.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): u.profile_image_url_https}" alt="${u.screen_name}" class="user-item-avatar tweet-avatar" width="48" height="48">
    ...
    try {
        ...
        tweet.innerHTML = /*html*/`
            ...
            <div class="tweet-body ${options.mainTweet ? 'tweet-body-main' : ''}">
                ...
                <a class="tweet-body-quote" target="_blank" href="https://twitter.com/${t.quoted_status.user.screen_name}/status/${t.quoted_status.id_str}">
                    <img src="${(t.quoted_status.user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): t.quoted_status.user.profile_image_url_https}" alt="${escapeHTML(t.quoted_status.user.name)}" class="tweet-avatar-quote" width="24" height="24">
        ...
        if(options.mainTweet) {
            ...
            for(let i in likers) {
                ...
                likerImg.src = `${(liker.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): liker.profile_image_url_https}`;
        ...
        t.renderFavoritesUp = () => {
            ...
            if(!options.mainTweet) {
                ...
                if(footerFavorites.children.length < 8 && !mainTweetLikers.find(liker => liker.id_str === user.id_str)) {
                    ...
                    let likerImg = document.createElement('img');
                    likerImg.src = `${(user.default_profile_image && vars.useOldDefaultProfileImage) ? chrome.runtime.getURL(`images/default_profile_normal.png`): user.profile_image_url_https}`;
                    likerImg.classList.add('tweet-footer-favorites-img');
