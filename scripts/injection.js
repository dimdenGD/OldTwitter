    ...
    : pages.find(
        (p) =>
          ...
          (p.paths.includes(realPath) ||
            p.paths.find((r) => r instanceof RegExp && r.test(realPath)))
      );
(async () => {
  if (!page) return;
  ...
  let [LOC_DATA, LOC_EN_DATA, html, css, header_html, header_css] =
    await Promise.all([
      fetch(chrome.runtime.getURL(`_locales/${LANGUAGE}/messages.json`)).then(
        (response) => response.json()
      ),
      fetch(chrome.runtime.getURL(`_locales/en/messages.json`)).then(
        (response) => response.json()
      ),
      fetch(chrome.runtime.getURL(`layouts/${page.name}/index.html`)).then(
