    ...
    user: {
        ...
        getTweetsV2: (id, cursor, replies = false, useDiffKey) => {
            return new Promise((resolve, reject) => {
                ...
                let variables = {"userId":id,"count":20,"includePromotedContent":true,"withCommunity":true,"withVoice":true,"withV2Timeline":true};
                ...
                fetch(`/i/api/graphql/${api}?variables=${encodeURIComponent(JSON.stringify(variables))}&features=${encodeURIComponent(JSON.stringify(features))}${fieldToggles ? `&fieldToggles=${encodeURIComponent(JSON.stringify(fieldToggles))}` : ''}`, {
                    headers: {
                        "content-type": "application/json",
                        "authorization": useDiffKey ? OLDTWITTER_CONFIG.oauth_key : OLDTWITTER_CONFIG.public_token,
                        "x-csrf-token": OLDTWITTER_CONFIG.csrf,
                    ...
                    r.text().then(data => {
                        ...
                        let ix = data.data?.user?.result?.timeline_v2?.timeline?.instructions;
                        if(!ix) ix = data.data?.user?.result?.timeline?.timeline?.instructions;
