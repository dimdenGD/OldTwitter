...
function parseNoteTweet(result) {
  ...
  if (result.note_tweet.note_tweet_results.result) {
    ...
    if (
      result.note_tweet.note_tweet_results.result.richtext?.richtext_tags.length
    ) {
      entities.richtext =
...
const API = {
        ...
        fetch(`/i/api/graphql/U0cdisy7QFIoTfu3-Okw0A/HomeLatestTimeline`, {
          ...
          .then(async (data) => {
            ...
            if (tweets.length === 0 && messagePromptIndex === 0 && !cursor) {
              let messagePrompt =
                entries[messagePromptIndex].content.itemContent.content;
              if (
            ...
            let out = {
              ...
              suspended: entries.find(
                (e) => e.entryId === "messageprompt-suspended-prompt"
              ),
            };
            debugLog("timeline.getChronologicalV2", "end", {
              cursor,
          ...
          .then((data) => {
            ...
            for (let i = 0; i < entries.length; i++) {
              ...
              if (
                ...
                continue;
              if (e.feedbackInfo) {
                tweet.feedback = e.feedbackInfo.feedbackKeys.map(
                  (f) => data.timeline.responseObjects.feedbackActions[f]
                );
                if (tweet.feedback)
                  tweet.feedbackMetadata = e.feedbackInfo.feedbackMetadata;
              }
              if (tweet.retweeted_status_id_str) {
                tweet.retweeted_status = tweets[tweet.retweeted_status_id_str];
                if (tweet.retweeted_status) {
                  tweet.retweeted_status.user =
                    users[tweet.retweeted_status.user_id_str];
          ...
          .then((data) => {
            ...
            if (tweets.length === 0 && messagePromptIndex === 0 && !cursor) {
              ...
              if (
                ...
                localStorage.hitRateLimit = Date.now() + 1000 * 60 * 10;
                return API.timeline
                  .getAlgorithmicalV2(cursor, count, seenTweetIds, true)
                  .then(resolve)
                  .catch(reject);
              }
            }
        ...
        chrome.storage.local.get(["algoTimeline"], (d) => {
          if (d.algoTimeline && Date.now() - d.algoTimeline.date < 60000 * 3) {
            if (d.algoTimeline.data && d.algoTimeline.data.list) {
              d.algoTimeline.data.list = d.algoTimeline.data.list.filter(
      ...
      for (let i = chrono.list.length - 1; i >= 0; i--) {
        ...
        if (i % 7 === 0) {
          ...
          if (
            ...
            chrono.list[chrono.list.length - i - 1].threadContinuation
          ) {
            continue;
          }
      ...
      return new Promise((resolve, reject) => {
        chrome.storage.local.get([`peopleRecommendations`], (d) => {
          ...
          if (
            ...
            Date.now() -
              d.peopleRecommendations[`${id}${by_screen_name}`].date <
              60000 * 7
          ) {
            debugLog(
              "discover.getSimilarPeople",
              "cache",
              d.peopleRecommendations[`${id}${by_screen_name}`].data
            );
            return resolve(
              d.peopleRecommendations[`${id}${by_screen_name}`].data
            );
          ...
          fetch(
            ...
            .then((data) => {
              ...
              chrome.storage.local.set(
                { peopleRecommendations: d.peopleRecommendations },
                () => {}
              );
            })
            .catch((e) => {
            ...
            .then((data) => {
              ...
              for (let i in entries) {
                ...
                if (e.content.notification) {
                  let n =
                    data.globalObjects.notifications[e.content.notification.id];
                  if (!n) continue;
                  if (e.feedbackInfo) {
                    n.feedback =
                    ...
                    if (n.feedback)
                      n.feedback.metadata = e.feedbackInfo.feedbackMetadata;
                  }
                  let tweet = n.template.aggregateUserActionsV1.targetObjects[0]
                    ? data.globalObjects.tweets[
                        n.template.aggregateUserActionsV1.targetObjects[0].tweet
                          .id
                      ]
                    : { full_text: "" };
                  if (tweet && tweet.user_id_str) {
                    if (tweet.quoted_status_id_str) {
                      tweet.quoted_status =
                        data.globalObjects.tweets[tweet.quoted_status_id_str];
                      if (tweet.quoted_status) {
                        tweet.quoted_status.user =
                  ...
                  if (t.quoted_status_id_str) {
                    t.quoted_status =
                      data.globalObjects.tweets[t.quoted_status_id_str];
                    if (t.quoted_status)
                      t.quoted_status.user =
                        data.globalObjects.users[t.quoted_status.user_id_str];
                  }
                  ...
                  if (
                    ...
                    d.notifications.data.list =
                      d.notifications.data.list.filter(
                        (n) =>
                          !res.find(
                            (r) => r.type === "notification" && r.id === n.id
                          )
                      );
                    d.notifications.data.list = res.concat(
                      d.notifications.data.list
                    );
    ...
    getDeviceFollowTweets: (cursor) => {
      return new Promise((resolve, reject) => {
        fetch(
          ...
          .then((i) => i.json())
          .then((data) => {
            debugLog("notifications.getDeviceFollowTweets", "start", {
              cursor,
              data,
            });
            if (data.errors && data.errors[0].code === 32) {
            ...
            for (let i in tweets) {
              ...
              if (tweets[i].retweeted_status_id_str) {
                tweets[i].retweeted_status =
                  data.globalObjects.tweets[tweets[i].retweeted_status_id_str];
                if (tweets[i].retweeted_status)
                  tweets[i].retweeted_status.user =
                    data.globalObjects.users[
                      tweets[i].retweeted_status.user_id_str
                    ];
              }
            }
        ...
        fetch(
          ...
          .then((data) => {
            ...
            for (let i in entries) {
              ...
              if (e.entryId.startsWith("tweet-")) {
                let tweet =
                  data.globalObjects.tweets[e.content.item.content.tweet.id];
                ...
                if (tweet.quoted_status_id_str) {
                  ...
                  if (tweet.quoted_status) {
                    tweet.quoted_status.user =
                      data.globalObjects.users[tweet.quoted_status.user_id_str];
                  }
                }
                if (tweet.retweeted_status_id_str) {
                  tweet.retweeted_status =
                    data.globalObjects.tweets[tweet.retweeted_status_id_str];
                  if (tweet.retweeted_status) {
                    tweet.retweeted_status.user =
                      data.globalObjects.users[
                        tweet.retweeted_status.user_id_str
                ...
                let id =
                  e.content.timelineModule.items[0].item.content.tweet.id;
        ...
        fetch(`https://api.${location.hostname}/1.1/friendships/create.json`, {
          ...
          .then((data) => {
            ...
            chrome.storage.local.get(["sortedFollowers"], async (d) => {
              ...
              let index = sortedFollowers[user.id_str].followers.findIndex(
                (f) => f[2] === screen_name
              );
              if (index === -1) return;
            ...
            let cachedUser = Object.values(userStorage).find(
              (u) => u.screen_name.toLowerCase() === screen_name.toLowerCase()
            );
            if (cachedUser) {
        ...
        fetch(
          ...
          r.text()
            .then((data) => {
              ...
              if (!ix)
                ix = data.data?.user?.result?.timeline?.timeline?.instructions;
      ...
      return new Promise((resolve, reject) => {
        ...
        fetch(
          ...
          .then((data) => {
            ...
            let isGrid =
              !!entries.find((e) => e.entryId === "profile-grid-0") ||
              data.data.user.result.timeline_v2.timeline.instructions.find(
            ...
            if (isGrid) {
              ...
              if (cursor) {
                ...
                items =
                  entries.find((e) => e.entryId === "profile-grid-0")?.content
                    ?.items || [];
              }
              tweets = items
              ...
              tweets = entries
                ...
                .map((t) => {
                  let tweet = parseTweet(
                    t.content.itemContent.tweet_results.result
                  );
                  if (tweet) {
                    tweet.hasModeratedReplies =
          ...
          .then((data) => {
            ...
            let out = {
              tl: data.data.user.result.timeline_v2.timeline.instructions[0].entries
                ...
                .filter((e) => e),
              cursor:
                data.data.user.result.timeline_v2.timeline.instructions[0].entries.find(
        ...
        fetch(
          ...
          .then((data) => {
            ...
            resolve({
              list: data.users,
              cursor:
                data.next_cursor_str !== "0" ? data.next_cursor_str : null,
            });
          })
          .catch((e) => {
            reject(e);
          ...
          .then((data) => {
            ...
            resolve(
              data.data.user.result.timeline.timeline.instructions
                .find((i) => i.entries)
                .entries.filter((e) => e.entryId.startsWith("list-"))
  ...
  tweet: {
            ...
            .then((data) => {
                ...
                if (data.errors[0].code === 88 && !useDiffKey) {
                  ...
                  API.tweet
                    ...
                    .then((t) => {
                      resolve(t);
                      if (loadingDetails[id])
                        loadingDetails[id].listeners.forEach((l) => l[0](t));
                      delete loadingDetails[id];
                    })
                    .catch((e) => {
                      reject(e);
                      if (loadingDetails[id])
                        loadingDetails[id].listeners.forEach((l) => l[1](e));
                      delete loadingDetails[id];
                    });
                  return;
                }
              ...
              let ic =
                data.data.threaded_conversation_with_injections_v2.instructions
                  ...
                  .entries.find((e) => e.entryId === `tweet-${id}`)
                  .content.itemContent;
    ...
    vote: (api, tweet_id, card_uri, card_name, selected_choice) => {
      return new Promise((resolve, reject) => {
        fetch(
          `https://caps.${location.hostname}/v2/capi/${api.split("//")[1]}`,
          {
            headers: {
              authorization: OLDTWITTER_CONFIG.public_token,
              "x-csrf-token": OLDTWITTER_CONFIG.csrf,
              ...
              for (let i = 0; i < entries.length; i++) {
                ...
                if (e.entryId.startsWith("tweet-")) {
                  ...
                  if (tweet.quoted_status_id_str) {
                    ...
                    if (tweet.quoted_status) {
                      tweet.quoted_status.user =
                        userData[tweet.quoted_status.user_id_str];
                      tweet.quoted_status.user.id_str =
                        tweet.quoted_status.user_id_str;
                      tweet.quoted_status.id_str = tweet.quoted_status_id_str;
                    }
                  ...
                  if (e.content.item.content.tombstone.tweet) {
                    let tweet =
                      tweetData[e.content.item.content.tombstone.tweet.id];
                    let user = userData[tweet.user_id_str];
                    tweet.id_str = e.content.item.content.tombstone.tweet.id;
                    tweet.user = user;
                    if (tweet.quoted_status_id_str) {
                      tweet.quoted_status =
                        tweetData[tweet.quoted_status_id_str];
                      if (tweet.quoted_status) {
                        tweet.quoted_status.user =
                          userData[tweet.quoted_status.user_id_str];
                        tweet.quoted_status.user.id_str =
                  ...
                  for (let j = 0; j < thread.length; j++) {
                    let t = thread[j];
                    let tweet =
                      tweetData[t.conversationTweetComponent.tweet.id];
                    if (!tweet) continue;
                    let user = userData[tweet.user_id_str];
                  ...
                  e.entryId.startsWith("cursor-showmorethreadsprompt")
                ) {
                  ...
                  list.push({
                    ...
                    data: {
                      ...
                      labelText:
                        e.content.itemContent.displayTreatment.labelText,
                      actionText:
                        e.content.itemContent.displayTreatment.actionText,
                    },
                  });
                }
              }
                ...
                if (data.errors[0].code === 88 && !useDiffKey) {
                  ...
                  API.tweet
                    ...
                    .then((t) => {
                      resolve(t);
                      if (loadingReplies[id])
                        loadingReplies[id].listeners.forEach((l) => l[0](t));
                      delete loadingReplies[id];
                    })
                    .catch((e) => {
                      reject(e);
              ...
              for (let i = 0; i < entries.length; i++) {
                ...
                if (e.entryId.startsWith("tweet-")) {
                  ...
                  if (tweetData.tombstone) {
                    ...
                    let replyTweet = entries.find(
                      (i) =>
                        ...
                        i.content.itemContent.tweet_results.result.legacy
                          .in_reply_to_status_id_str == tombstoneTweetId
                    );
                    list.push({
                      type: "tombstone",
                      data: text,
                  ...
                  if (tweet) {
                    if (
                      !tweet.id_str === id &&
                      (tweet.user.blocking || tweet.user.muting)
                    )
                      continue;
                    tweet.hasModeratedReplies =
                    ...
                    if (thread[j].entryId.includes("cursor-showmore")) {
                      list.push({
                        ...
                        data: {
                          ...
                          labelText:
                            thread[j].item.itemContent.displayTreatment
                              .labelText,
                          actionText:
                            thread[j].item.itemContent.displayTreatment
                              .actionText,
                  ...
                  e.entryId.startsWith("cursor-showmorethreadsprompt") ||
                  e.entryId.startsWith("cursor-showmorethreads-")
                ) {
                  let value =
                    e?.content?.itemContent?.value ?? e?.content?.value;
                  if (!value) continue;
                  if (newCursor === value) {
                    continue;
                  }
                ...
                list: list
                  ...
                  .map((e) => {
                    if (
                      ...
                      e.content.itemContent.user_results.result.__typename ===
                        "UserUnavailable"
                    )
                      return;
                    let user = e.content.itemContent.user_results.result;
      ...
      return new Promise((resolve, reject) => {
        ...
        fetch(
          ...
          .then((data) => {
            ...
            let list = data.data.retweeters_timeline.timeline.instructions.find(
              (i) => i.type === "TimelineAddEntries"
            );
              ...
              let replacerEntry =
                data.data.search_by_raw_query.search_timeline.timeline.instructions.find(
                  (i) =>
                    ...
                    i.entry_id_to_replace.startsWith("cursor-bottom-")
                );
              if (replacerEntry) {
            ...
            if (!cursor) {
              let entries =
                data.data.tweet.result.timeline_response.timeline.instructions.find(
                  (i) =>
                    i.replaceEntry &&
                    i.replaceEntry.entryIdToReplace.includes("cursor-bottom")
                );
              if (entries) {
                cursor =
                  entries.replaceEntry.entry.content.operation.cursor.value;
              }
            } else {
  ...
  search: {
    ...
    adaptive: (obj, cursor) => {
      ...
      return new Promise((resolve, reject) => {
        fetch(
          `/i/api/2/search/adaptive.json?${cursor ? `cursor=${cursor}&` : ""}${
            obj.tweet_search_mode
              ? `tweet_search_mode=${obj.tweet_search_mode}&`
              : ""
          }include_profile_interstitial_type=1&include_blocking=1&include_blocked_by=1&include_followed_by=1&include_want_retweets=1&include_mute_edge=1&include_can_dm=1&include_can_media_tag=1&include_ext_has_nft_avatar=1&skip_status=1&cards_platform=Web-12&include_cards=1&include_ext_alt_text=true&include_quote_count=true&include_reply_count=1&tweet_mode=extended&include_ext_collab_control=true&include_entities=true&include_user_entities=true&include_ext_media_color=true&include_ext_media_availability=true&include_ext_sensitive_media_warning=true&include_ext_trusted_friends_metadata=true&send_error_codes=true&simple_quoted_tweet=true&q=${
            obj.q
          }${obj.social_filter ? `&social_filter=${obj.social_filter}` : ""}${
            obj.result_filter ? `&result_filter=${obj.result_filter}` : ""
          ...
          .then((data) => {
            ...
            if (cursorTop) {
              ...
              cursorTop = instructions.find(
                (e) =>
                  e.entry_id_to_replace &&
                  (e.entry_id_to_replace.startsWith("sq-cursor-top-") ||
                    e.entry_id_to_replace.startsWith("cursor-top-"))
              );
              if (cursorTop) {
        ...
        chrome.storage.local.get(["listData"], (d) => {
          ...
          fetch(
            `/i/api/graphql/vxx-Y8zadpAP64HHiw4hMQ/ListByRestId?variables=${encodeURIComponent(
              JSON.stringify({ listId: id, withSuperFollowsUserFields: true })
            )}&features=${encodeURIComponent(
              JSON.stringify({
                responsive_web_graphql_timeline_navigation_enabled: false,
              })
    ...
    subscribe: (id) => {
      return new Promise((resolve, reject) => {
        fetch(`/i/api/graphql/nymTz5ek0FQPC3kh63Tp1w/ListSubscribe`, {
          ...
          method: "POST",
          body: JSON.stringify({
            variables: { listId: id, withSuperFollowsUserFields: true },
            features: {
              responsive_web_graphql_timeline_navigation_enabled: false,
            },
            ...
            .then((data) => {
              ...
              let out =
                data.data.viewer.list_management_timeline.timeline.instructions
                  .find((i) => i.entries)
                  .entries.find((i) =>
                    i.entryId.startsWith("owned-subscribed-list-module")
                  )
          ...
          .then((data) => {
            ...
            resolve(
              data.data.trusted_friends_list_by_rest_id.members_slice.items_results
                .filter(
                  (u) => u.result && u.result.is_trusted_friends_list_member
                )
                .map((u) => u.result)
            );
          })
